...
export function RainbowKitAuthenticationProvider<Message = unknown>({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: handleChangedAccount is stable and including it causes unnecessary re-renders
  useEffect(() => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: adapter is stable and connector?.uid is intentionally compared against currentConnectorUid
  useEffect(() => {
    if (
      currentConnectorUid &&
      typeof connector?.emitter?.on === 'function' &&
